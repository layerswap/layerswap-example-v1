...
function App() {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.layerswap.io/api/swap_rate?version=mainnet', {
        method: 'POST',
        headers: {
  ...
  const createSwap = async () => {
    try {
      ...
      const response = await fetch('https://api.layerswap.io/api/swaps', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      const fetchedSwapData = await response.json();

      try {
        const res = await fetch(`https://api.layerswap.io/api/swaps/${fetchedSwapData?.data?.swap_id}/prepare_src_transaction`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'X-LS-APIKEY': 'sK7bkuFZ/AYsU/f6D1+Qgrkm7qDF/ewRF3kWfMJ2jFFbk6ctKyVaD7SWL5mRyKI2kqAd7gVV2frQGphLZCnISg'
  ...
  return (
    <div className="App h-screen flex items-center justify-center">
                                ...
                                <div className="relative">
                                  <div className="rounded-lg focus-peer:ring-primary focus-peer:border-secondary-400 focus-peer:border focus-peer:ring-1 focus:outline-none disabled:cursor-not-allowed relative grow h-12 flex items-center text-left justify-bottom w-full pl-3 pr-2 py-2 bg-secondary-600 border border-secondary-500 font-semibold align-sub ">
                                    <div className="disabled:cursor-not-allowed relative grow flex items-center text-left w-full font-semibold">
                                      ...
                                      <span className="flex grow text-left items-center"><span className="block text-xs md:text-base font-medium text-primary-text-placeholder flex-auto items-center">USDC</span></span></div>
                                  </div>
                                </div>
                              </div>
                            </div>
      ...
      {showSwapDialog &&
        <SwapDialog open={showSwapDialog} toggleOpen={setShowSwapDialog} address={addressField} swapData={swapData} />
      }
    </div>
  );
}
